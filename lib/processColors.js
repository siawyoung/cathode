...
var postDir   = rootDir + "content/post"
var imageDir  = rootDir + "static"
var dataDir   = rootDir + "data"

fs.readdir(postDir, function(err, list) {
  var numberOfFiles = list.length
  var toWrite = new Map()
  // var asd = new Map()
  list.forEach(function(post) {

    fs.readFile(postDir + "/" + post, "utf-8", function(err, data) {
        ...
        fs.readFile(path, function(err, data) {
          ...
          img.onload = function() {
            ...
            if (toWrite.size === numberOfFiles) {
              fs.writeFile(dataDir + "/" + "colors.json", JSON.stringify(strMapToObj(toWrite)))
            }
          }
          img.src = data
        })
